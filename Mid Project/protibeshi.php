<!DOCTYPE html>
<html>
<head>
	<title>Protibeshi</title>
    <style>
        #protibeshi {
            border: 2px solid #0aab8e !important;
        }
    </style>
</head>
<body>
	<?php
    include 'nav.html';
    ?>
    <div style="display: block; width: 1px; margin: 0 auto">
    	<p style="display: inline-block; border: .5px solid #0aab8e; height: 100px"></p>
    </div>
    <h1 style="text-align: center; ">Welcome at PROTIBESHI</h1>
    <a href="registration.php" style="text-align: center; color: #0aab8e; font-family: Roboto; text-decoration: none; width: 100px; display: block; margin: 0 auto 30px auto">JOIN US</a>
    <div style="display: block; width: 1px; margin: 0 auto">
    	<p style="display: inline-block; border: .5px solid #0aab8e; height: 100px"></p>
    </div>
</body>
</html>